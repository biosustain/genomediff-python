...
RA	2		NC_000913	223	0	G	A	frequency=0.1366
        ...
        p = GenomeDiffParser(file)

        self.assertEqual([
                             Metadata('GENOME_DIFF', '1.0'),
                             Metadata('AUTHOR', 'test'),
                             Record('SNP', 1, [23423], new_seq='A', seq_id='NC_000913', position=223, gene_name='mhpE'),
                             Record('RA', 2, None, new_base='A', frequency=0.1366, position=223, seq_id='NC_000913',
                                    insert_position=0,
                                    ref_base='G')],
                         list(p)
        )
...
class RecordTestCase(TestCase):
    def test_simple(self):
        snp_record = Record('SNP', 1, [23423], seq_id='NC_000913', new_seq='A', position=223, test='more')
